...
async function place(pool,userId,{matchId,mapCount,stakePoints,expectedOdds}={}){
  ...
  return transaction(pool,async client=>{
    const m=(await client.query("SELECT * FROM matches WHERE id=$1 AND status='open' AND winner IS NULL AND starts_at>NOW()+INTERVAL '10 minutes' FOR UPDATE",[matchId])).rows[0];
...
async function setOdds(pool,id,values){
  return transaction(pool,async client=>{
    ...
    const open=(await client.query("SELECT id FROM matches WHERE id=$1 AND status='open' AND winner IS NULL AND starts_at>NOW()+INTERVAL '10 minutes'",[id])).rows.length;
...
async function undoLocked(client,id){
  const preds=(await client.query('SELECT * FROM map_predictions WHERE match_id=$1 AND result IS NOT NULL ORDER BY user_id,id FOR UPDATE',[id])).rows;
  for(const p of preds){
...
async function undo(pool,id){return transaction(pool,async client=>{await lockMatch(client,id);return undoLocked(client,id)})}
