...
async function refund(pool,id,reason='canceled',sourceMatch=null){
  ...
  return transaction(pool,id,async(c,m)=>{
    ...
    // Explicit type avoids conflicting inference across varchar and text columns.
    await c.query('UPDATE matches SET status=$1::text,source_status=$1::text,predictions_voided_at=NOW(),void_reason=$1::text,synced_at=NOW() WHERE id=$2',[reason,m.id]);
