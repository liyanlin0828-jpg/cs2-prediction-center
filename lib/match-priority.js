...
// Directory coverage is independent of match ordering and is not a world ranking.
const directoryNames=['Aurora','Aurora Gaming','PARIVISION','BetBoom','BetBoom Team','SAW','Monte','9z','9z Team','Legacy','FUT','FUT Esports','The Mongolz','Mongolz'];
const featuredTeamNames=[...teamGroups.flatMap(g=>g.names),...directoryNames];
const directoryTeams={...Object.fromEntries(directoryNames.map(name=>[normalize(name),40])),...teams};
const teamScore=name=>directoryTeams[normalize(typeof name==='string'?name:'')]||0;
const directoryAliases={teamvitality:'vitality',natusvincere:'navi',fazeclan:'faze',g2esports:'g2',teamspirit:'spirit',furiaesports:'furia',teamfalcons:'falcons',teamliquid:'liquid',vp:'virtuspro',themongolz:'mongolz',lynnvisiongaming:'lynnvision',nip:'ninjasinpyjamas',complexitygaming:'complexity',paingaming:'pain',imperialesports:'imperial',nrgesports:'nrg',auroragaming:'aurora',betboomteam:'betboom',futesports:'fut','9zteam':'9z'};
const teamKey=name=>{const key=normalize(name);return directoryAliases[key]||key};
module.exports={scoreSql,orderSql,featuredTeamNames,teamScore,teamKey};
