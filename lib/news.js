'use strict';
const SOURCES=[
 {id:'hltv',label:'HLTV',category:'esports',interval:60*60*1000,url:'https://www.hltv.org/rss/news'},
...
function safeUrl(value,source){try{const u=new URL(value);const hosts=source==='hltv'?['www.hltv.org','hltv.org']:['store.steampowered.com','steamcommunity.com','steamstore-a.akamaihd.net'];return u.protocol==='https:'&&!u.username&&!u.password&&hosts.includes(u.hostname)?u.href:null}catch{return null}}
function parseFeed(text,source){
 ...
 if(source==='steam'){
  ...
  raw=[...text.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)].map(([,x])=>({id:field(x,'guid')||field(x,'link'),title:field(x,'title'),url:field(x,'link'),date:Date.parse(field(x,'pubDate'))}));
 }
 const seen=new Set();return raw.flatMap(x=>{
  ...
  seen.add(url);return [{id:source+':'+x.id,title,url,publishedAt:new Date(x.date).toISOString(),source,category:source==='hltv'?'esports':'official'}];
...
function createNewsService(pool,fetcher=fetch){
 ...
 async function sync(){
  ...
  running=(async()=>{
   for(const source of SOURCES){
    ...
    if(row?.checked_at&&Date.now()-new Date(row.checked_at).getTime()<source.interval)continue;
    try{
     const response=await fetcher(source.url,{signal:AbortSignal.timeout(12000),headers:{Accept:source.id==='hltv'?'application/rss+xml':'application/json'}});
     ...
     const items=parseFeed(text,source.id);if(!items.length)throw new Error('Empty news feed');
 ...
 async function read(){
  const rows=(await pool.query('SELECT * FROM news_feeds')).rows;
