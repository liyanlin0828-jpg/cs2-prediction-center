...
function createTeamService(pool,panda){
  ...
  async function sync(){
    ...
    running=(async()=>{
      ...
      try{
        ...
        )`)).rows.filter(m=>positiveId(m.external_id));
        ...
        for(let offset=0;offset<locals.length;offset+=100){
          ...
          for(const match of matches){
            ...
            links.push(link);ids.add(link.a);ids.add(link.b);
          }
        }
        ...
        try{
          ...
          await client.query('UPDATE team_sync_state SET success_at=NOW(),failed=$1 WHERE id=1',[profiles.size<teamIds.length||links.length<locals.length]);
  ...
  async function list(){const rows=(await pool.query('SELECT * FROM team_profiles ORDER BY data->>\'name\',id')).rows;return {teams:rows.map(present),sync:await status()}}
