...
function normalizeTeam(team){
  ...
  return {id,name:text(team.name),acronym:text(team.acronym),location:text(team.location),imageUrl:imageUrl(team.image_url),rosterKnown:players!==null,sourceUpdatedAt:Number.isFinite(Date.parse(team.modified_at))?new Date(team.modified_at).toISOString():null,
    players:(players||[]).flatMap(p=>{const id=positiveId(p?.id);if(!id||!text(p.name)||seen.has(id))return [];seen.add(id);return [{id,nickname:text(p.name),nationality:text(p.nationality),imageUrl:imageUrl(p.image_url),sourceActive:typeof p.active==='boolean'?p.active:null,sourceUpdatedAt:Number.isFinite(Date.parse(p.modified_at))?new Date(p.modified_at).toISOString():null}]})};
}
function createTeamService(pool,panda,ranking=rankingTools.createRankingService(pool)){
  ...
  async function list(){
    ...
    const teams=ranked.teams.filter(t=>!rankingTools.academy(t.name)).map(entry=>{
      ...
      return {...(profile||{id:'hltv:'+entry.hltvId,players:[],rosterKnown:false}),rosterReference:{source:'HLTV ranking',date:ranked.date,url:ranked.sourceUrl,players:entry.players},name:entry.name,aliases:[...rankingTools.aliases(entry),...(profile?[profile.name]:[])],rank:entry.rank,hltvId:entry.hltvId,profileAvailable:!!profile};
    });
