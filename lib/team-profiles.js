...
function createTeamService(pool,panda){
  ...
  const present=row=>row&&priority.teamScore(row.data.name)?{...row.data,aliases:priority.featuredTeamNames.filter(name=>priority.teamKey(name)===priority.teamKey(row.data.name)),syncedAt:row.synced_at,stale:Date.now()-new Date(row.synced_at).getTime()>INTERVAL*2}:null;
