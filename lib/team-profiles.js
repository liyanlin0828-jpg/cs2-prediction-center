...
function normalizeTeam(team){
  ...
  const seen=new Set();
  return {id,name:text(team.name),acronym:text(team.acronym),location:text(team.location),imageUrl:imageUrl(team.image_url),rosterKnown:players!==null,
...
function createTeamService(pool,panda){
  ...
  async function sync(){
    ...
    running=(async()=>{
      ...
      if(previous?.checked_at&&Date.now()-new Date(previous.checked_at).getTime()<INTERVAL)return;
      ...
      try{
        ...
        try{
          ...
          for(let page=1;page<=10;page++){
            const teams=await panda('/csgo/teams?per_page=100&sort=id&page='+page+'&filter[name]='+encodeURIComponent(priority.featuredTeamNames.join(',')));
  ...
  async function list(){const rows=(await pool.query('SELECT * FROM team_profiles ORDER BY data->>\'name\',id')).rows;const teams=rows.filter(row=>priority.teamScore(row.data.name)).map(present);teams.sort((a,b)=>priority.teamScore(b.name)-priority.teamScore(a.name)||a.name.localeCompare(b.name)||a.id-b.id);return {teams,sync:await status()}}
