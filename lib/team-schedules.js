...
function createScheduleService(pool,panda){
 let running=null;
 async function tick(teams){if(running)return running;running=(async()=>{
  const states=new Map((await pool.query('SELECT team_id,checked_at FROM team_match_cache')).rows.map(r=>[Number(r.team_id),r]));
  const due=teams.filter(t=>t.profileAvailable&&id(t.id)&&(!states.get(Number(t.id))?.checked_at||Date.now()-new Date(states.get(Number(t.id)).checked_at).getTime()>=INTERVAL));
