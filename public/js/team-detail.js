(()=>{
 ...
 async function load(){if(busy)return;if(!/^[1-9]\d*$/.test(id||'')||!Number.isSafeInteger(Number(id))){el('teamTitle').textContent='无效战队链接';el('detailStatus').textContent='请从全部战队页面选择战队。';refresh.disabled=true;return}
  busy=true;refresh.disabled=true;
  try{const r=await fetch('/api/teams/'+id,{signal:AbortSignal.timeout(15000)});if(r.status===404){el('teamTitle').textContent='该战队不在当前榜单中';el('detailStatus').textContent='请返回全部战队查看最新名单。';el('teamProfile').replaceChildren();el('teamUpcoming').replaceChildren();el('teamResults').replaceChildren();el('detailRanking').replaceChildren();return}if(!r.ok)throw Error();const d=await r.json();
   ...
   for(const [target,items,result] of [['teamUpcoming',d.upcoming,false],['teamResults',d.results,true]])el(target).replaceChildren(...(items.length?items.map(m=>matchCard(m,result)):[node('p',result?'本站暂无已同步赛果。':'本站暂无已同步赛程。','muted')]));loaded=true;
